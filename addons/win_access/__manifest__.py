{
    'name': 'Windows Access Manager',
    'version': '19.0.1.0.0',
    'category': 'Administration',
    'summary': 'Manage groups + members across AD, local Windows, SSAS and PBIRS via the Windows Access Management API',
    'description': """
Windows Access Manager
======================
Manages group membership across Active Directory, local Windows groups,
SSAS and Power BI Report Server through the external Windows Access
Management HTTP API, plus BI row-level security driven by ``MD.UserAccess``
(see ``docs/BI_ACCESS.md``).
""",
    'author': 'ShakaERP',
    'maintainer': 'ShakaERP',
    'website': 'https://shakasystem.com',
    'license': 'LGPL-3',
    'icon': '/win_access/static/description/icon.svg',
    'depends': ['base', 'web', 'entity', 'shaka_ui_kit'],
    'external_dependencies': {'python': ['requests', 'pymssql']},
    'data': [
        'security/ir.model.access.csv',
        'views/win_access_views.xml',
        'views/bi_user_access_views.xml',
    ],
    'assets': {
        'web.assets_backend': [
            'win_access/static/src/win_access.scss',
            'win_access/static/src/pbirs_tree.js',
            'win_access/static/src/pbirs_tree.xml',
            'win_access/static/src/sync_panel.js',
            'win_access/static/src/sync_panel.xml',
            'win_access/static/src/member_list.js',
            'win_access/static/src/member_list.xml',
            'win_access/static/src/loading_field.js',
            'win_access/static/src/loading_field.xml',
        ],
    },
    'installable': True,
    'application': True,
    'auto_install': False,
}
